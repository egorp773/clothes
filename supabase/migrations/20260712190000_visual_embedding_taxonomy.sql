-- Persist the FashionSigLIP taxonomy next to each embedding. Legacy products
-- often have Russian free-form categories and an empty item_type, so retrieval
-- must not rely on those fields alone.
...
alter table public.product_visual_embeddings
  ...
  add column if not exists detected_item_type text,
  add column if not exists detected_category_confidence double precision
    check (
      detected_category_confidence is null
      or detected_category_confidence between 0 and 1
    );

create index if not exists product_visual_embeddings_taxonomy_idx
