...
drop policy if exists "Participants can read profile follows" on public.profile_follows;
...
drop policy if exists "Users can follow sellers" on public.profile_follows;
create policy "Users can follow sellers"
  on public.profile_follows for insert
  to authenticated
  with check ((select auth.uid()) = follower_id and follower_id <> seller_id);

drop policy if exists "Users can unfollow sellers" on public.profile_follows;
...
drop trigger if exists refresh_profile_followers_count on public.profile_follows;
